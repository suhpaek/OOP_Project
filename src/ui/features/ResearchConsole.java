...
public class ResearchConsole implements ConsoleScreen {
    ...
    private void printMenu() {
        ...
        System.out.println("7. View all papers sorted");
    ...
    private List<ResearchPaper> getSortedPapers() {
        ...
        String choice = scanner.nextLine().trim();
        SortStrategy strategy = chooseSortStrategy(choice);
        return strategy == null ? researchService.getPapers(user) : researchService.sortPapers(user, strategy);
    }

    private SortStrategy chooseSortStrategy(String choice) {
        if ("1".equals(choice)) return new SortByCitations();
        if ("2".equals(choice)) return new SortByDate();
        if ("3".equals(choice)) return new SortByPages();
        return null;
    }
    ...
    private void publishPaper() {
        try {
            ...
            paper.setCitations(Integer.parseInt(scanner.nextLine().trim()));
    ...
    private void viewAllPapers() {
        System.out.print("Sort by (1 citations, 2 date, 3 pages, other none): ");
        List<ResearchPaper> papers = researchService.getAllPapersSorted(chooseSortStrategy(scanner.nextLine().trim()));
        if (papers.isEmpty()) {
            System.out.println("No papers found.");
            return;
        }
